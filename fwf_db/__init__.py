...
key features:
    ...
    - May be not a big deal, but newlines can be any chars, including \x00 or \x01
    - May be not a big deal, but the last line can have the newline missing
    - Extensible: add you own views or indexes or ... We hope you find this little
      lib useful to build our own functionalities on top.
    - Treat the fixed width data like a table or database, and as if the data were
      in memory, even though they are not. E.g. [0], [-1], [:], [0:-1], etc.. They
      are called views in the lib. Currenly view implementations exist for ranges 
      (start, stop) and indices ([1, 4, 5, 20, 1020])
    - Find all unique (distinct) values in a column. The lib contains already
      two different implementations. One based on standatd python, the
      other leveraging Numpy and Pandas.
    - The same for indexes. Already two implementations exists. The index is kept
      in memory and allows easy and fast access to the record(s) it is referencing. 
      No matter how much memory you have, the OS pulls in the data if needed or
      takes it from memory.
    ...
    - Often the data needed are in several different files, e.g. because 
      every day a new one arrives and you need to process that data for the 
      last X days. The lib contains a multi-file module that supports this use
      case and makes the data from all files, look like they were in one table. 
