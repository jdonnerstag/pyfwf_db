...
cdef struct InternalData:
    cdef int field1_start_len
    cdef int field1_start_stoppos

    cdef int field1_end_len
    cdef int field1_end_stoppos
    cdef int field1_end_lastpos

    cdef int field2_start_len
    cdef int field2_start_stoppos

    cdef int field2_end_len
    cdef int field2_end_stoppos
    cdef int field2_end_lastpos
...
def iter_and_filter(fwf,
    ...
    cdef int min_fwidth = fwf.fwidth - fwf.number_of_newline_bytes
    ...
    cdef const char* line = mm + fwf.start_pos   # Current line
    cdef const char* file_end = mm + fwf.fsize

    while (line + min_fwidth) < file_end:

        # Execute the effective data and period filters
        if cmp_start_value(line, field1_startpos, field1_start_len, field1_start_value):
            pass # print(f"{irow} - 1 - False")
        elif cmp_end_value(line, field1_endpos, field1_end_len, field1_end_lastpos, field1_end_value):
            pass # print(f"{irow} - 2 - False")
        elif cmp_start_value(line, field2_startpos, field2_start_len, field2_start_value):
            pass # print(f"{irow} - 3 - False")
        elif cmp_end_value(line, field2_endpos, field2_end_len, field2_end_lastpos, field2_end_value):
